...
# 3. Start your Hero (Execute a loop below). Your Hero will start moving randomly every 1sec.
while ($true) 
{
...
# Get-GameElementCollection is a core function to analyze game elements collections. Use -Element parameter to specify target list/collection
...
while ($true)
{
	...
	$GameTime++
	"`n NEW GameTime is " + $GameTime + " LastTimePotionPlaced was " + $LastTimePotionPlaced
	...
	# Get Hero's position
	# If bomb just been placed, Hero is 'PotionHero' game element
	$myPotionHero = Get-GameElementCollection -Element PotionHero
	If ($myPotionHero)
	{
		$x = $myPotionHero[0][0]
		$y = $myPotionHero[0][1]
	...
	$myHero = Get-GameElementCollection -Element Hero
	If ($myHero)
	{
		$x = $myHero[0][0]
		$y = $myHero[0][1]
	...
	If (($LastTimePotionPlaced + 5) -lt $GameTime)
	{
		# set the action 
		$myNextAction = "act"
		# store gametime of bombplace action 
		$LastTimePotionPlaced = $GameTime
...
isAt 32 15 None,Boom,PotionTimer1,Wall
